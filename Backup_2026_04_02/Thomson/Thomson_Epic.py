...
#-------------------------------------------------   Retry Logic With Single Folder Name, Reference ID & Error Message    ------------------------------------#
...
async def login(page, context):
    ...
    await page.locator("//span[@class='text' and text()='TSLIN01_PROD']").click() # OR USE   Example : page.select_option('//select[@id="Skills"]',label='AutoCAD')
        ...
        while retry < MAX_RETRIES:
            ...
            try:
                ...
                break
                    ...
                    # Go back to Home before retry
                    home_btn = page.locator("div.main-button:has-text('Home')")
                    
                    # try:
                    #     if await home_btn.is_visible(timeout=3000):
                    #         await home_btn.click()
                    #         print("Clicked Home before retry")

                    #         # Wait until activity table loads again
                    #         await page.get_by_text("Follow Up/Start").wait_for(
                    #             state="visible", timeout=20000
                    #         )

                    # except:
                    #     print("Home button not visible")

                    if await home_btn.is_visible():
                        await home_btn.click()
                        print("Clicked Home before retry")

                        # wait until table loads again
...
#     logger.info("Starting Reference ID : %s" ,next_id)
#     return next_id
...
#         await later_btn.wait_for(timeout=5000)
#         await later_btn.click()
#         logger.info("Later Reset clicked")
#     except:
#         pass
...
#                 logger.info("Extracted: %s", pdf_path)

#     except Exception as e:
#         logger.info("MSG Extraction Error: %s", e)
...
#     logger.info("Yesterday: %s", yesterday_str)
...
#         date_text = (await date_element.inner_text()).strip()
#         logger.info("Date Text: %s", date_text)
...
#                 logger.info("Processing Row: %s | Ref ID: %s", row, ref_id)
...
#                 # row_text = await desc_ele.inner_text()
#                 # logger.info("Row clicked: %s", row_text)
...
#                 await description.wait_for(timeout=10000)
#                 description_value = await description.get_attribute("data-value")
#                 logger.info("Description: %s", description_value)
...
#                 logger.info("Association: %s", association_value)
...
#                 logger.info("LOB: %s", lob)
#                 logger.info("Policy Number: %s", policy_no)
...
#                 account_name = page.locator("//div[text()='Account name']/following::div[@data-value][1]")
#                 account_name_value = await account_name.get_attribute("data-value")
#                 logger.info("Account Name: %s", account_name_value)

#                 lookup_code = page.locator("//div[text()='Lookup code']/following::div[@data-value][1]")
#                 lookup_code_value = await lookup_code.get_attribute("data-value")
#                 logger.info("Lookup Code: %s", lookup_code_value)
...
#                 await account_manager.wait_for(timeout=10000)
#                 account_manager_value = await account_manager.input_value()
#                 logger.info("Account Manager: %s", account_manager_value)
...
#                 await access.wait_for()
#                 await access.click()
#                 logger.info("Access clicked to download attachments")

#                 await page.locator("span.text.force-underline", has_text="Attachments").first.click()
#                 logger.info("Attachments clicked")
...
#                 logger.info("Total Files: %s", row_count)
...
#                             file_text = (await file_element.inner_text()).strip()
#                             logger.info("Processing: %s", file_text)
...
#                         except Exception as e:
#                             retry += 1
#                             logger.info(f"Error at row {i+1} retry {retry}: {e}")
#                             await page.wait_for_timeout(3000)
...
#                     if await home_btn.is_visible():
#                         await home_btn.click()
#                         logger.info("Clicked Home before retry")
...
#                     if retry >= MAX_RETRIES:
#                         logger.info(f"Skipping row {row} after {MAX_RETRIES} retries")
