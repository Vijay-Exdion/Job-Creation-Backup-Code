...
def update_tracker(reference_id, job_id=None, status=None, error=None):
    ...
    #----------------- Error message to appear once ---------------------------
    if (
        status == "Error"
        and error
        and "Exceeds the allowed size limit of 40 MB for this BrokerId" in error
    ):
        print(f"Tracker Updated -> {reference_id} | {status}")
    else:
        print(f"Tracker Updated -> {reference_id} | {status} | {error}")
...
#     error_message = page.get_by_text("Login error occurred.")
...
async def login(page, url, username, password):
    ...
    for attempt in range(3):
        try:
            await page.locator("button.microsoft-button").click(timeout=5000)
            break
        except Exception:
            if attempt == 2:
                raise
    ...
    try:
        ...
        await Yes_btn.wait_for(timeout=5000)
        await Yes_btn.click()
    except:
        pass
...
async def create_job(page, record, lob_mapping):
    ...
    if os.path.exists(folder_path):
        ...
        files = [f for f in os.listdir(folder_path) if f.lower().endswith((".pdf", ".docx", ".xls", ".doc"))]
    ...
    # ================= CLASSIFY FILES =================
        ...
        for file in files:
            ...
            # Ignore Certificate documents from CT/PT classification
            if "certificate" in file_lower:
                other_files.append(file)
                continue
        ...
        if file.lower().endswith(".pdf"):
            ...
            if file_size_mb > 100:
                update_tracker(
                    ref_id,
                    status="Error",
                    error=f"Unable to create Job ID. PDF '{file}' exceeds 100 MB ({file_size_mb:.2f} MB)."
                )

                print(f"PDF exceeds 100 MB: {file} ({file_size_mb:.2f} MB)")
                return None
    ...
    #----------------------------------------------------------------------------------------------------
    # File size exceeds 40 MB - DO NOT RETRY
    if "exceeds the allowed size limit of 40 mb for this brokerid" in msg_lower:
        await dialog.accept()

        update_tracker(
            ref_id,
            status="Error",
            error=f"Popup message: {message}"
        )
        # Return so no retry is attempted
        return None
    #----------------------------------------------------------------------------------------------------
...
async def run_istudio():
    ...
    async with async_playwright() as p:
            ...
            while attempt <= MAX_RETRIES:
                ...
                try:
                    ...
                    if job_id:
                        ...
                        #-------------------------------------------------------------------
                         # Error already handled inside create_job (e.g., 40 MB popup)
                        df = pd.read_excel(TRACKER_PATH)
                        status = df.loc[df["Reference ID"] == ref_id, "Status"].iloc[0]

                        if status == "Error":
                            print(f"Skipped retry for {ref_id} due to 'Error' status.")
                            break
                        #---------------------------------------------------------------------
        ...
        try:
            ...
            profile_btn = page.get_by_text("Carroll_upload", exact=True)
